...
function drawHexagon(x, y, size, opacity, glow) {
    ...
    if (glow > 0) {
        ...
        ctx.shadowColor = `rgba(155, 89, 182, ${glow})`;
    }
...
// Loop de animação
function animate() {
    // Gradiente radial de fundo (centro mais claro)
    ...
    // Desenhar hexágonos com efeito de proximidade do mouse
    hexagons.forEach(hex => {
        if (mouse.x && mouse.y) {
            const dist = Math.hypot(hex.x - mouse.x, hex.y - mouse.y);
            if (dist < 150) {
                // Hexágono próximo ao mouse brilha mais
                hex.glowIntensity = Math.max(0, 1 - dist / 150);
...
// Rastrear mouse para efeito interativo
...
// Função para inicializar a animação do texto
function initTextAnimation() {
    const titleEl = document.getElementById("titleText");
    const subtitleEl = document.getElementById("subtitle");
    const buttonEl = document.getElementById("enterButton");
    ...
    function typeEffect() {
        if (index < finalText.length) {
            ...
            // Após terminar de digitar
            setTimeout(() => {
                titleEl.classList.add("glow-pulse");
                
                // Move para o topo
                setTimeout(() => {
                    titleEl.classList.add("moveToTop");
